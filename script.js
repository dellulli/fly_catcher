...
let score = 0;
const totalFlies = 30; // Goal to catch 35 flies
...
function startGame() {
    ...
    // Set up pencilholder dragging for both mouse and touch
    let isDragging = false;

const startDragging = (e) => {
    isDragging = true;
    e.preventDefault(); // Prevent default action to stop page scroll on mobile
};
...
const dragPencilHolder = (e) => {
    if (isDragging) {
        const x = e.pageX || e.touches[0].pageX;
        const y = e.pageY || e.touches[0].pageY;
        pencilholder.style.left = `${x - pencilholder.width / 2}px`;
        pencilholder.style.top = `${y - pencilholder.height / 2}px`;
        ...
        e.preventDefault(); // Prevent page from moving up/down on mobile
    }
};

pencilholder.addEventListener('mousedown', startDragging);
document.addEventListener('mouseup', stopDragging);
document.addEventListener('mousemove', dragPencilHolder);

pencilholder.addEventListener('touchstart', startDragging);
document.addEventListener('touchend', stopDragging);
document.addEventListener('touchmove', dragPencilHolder);
...
function endGame(won, specialItemSrc) {
    ...
    if (won) {
        ...
        if (specialItemSrc && specialItemSrc.includes('vapeleak.png')) {
            ...
            message.textContent = 'Maybe one day you\'ll actually be able to beat dellulli the pro fly catcher 😈';
        }
    }
